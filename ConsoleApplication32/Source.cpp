...
public:
	LivingPerson();                                         /*constructor*/
	LivingPerson(char*);                                    /*paremterized constructor*/
	~LivingPerson();                                        /*destructor*/
...
class Monster :public LivingPerson                       /*inheritance*/
{
private:
	Items *killedby;
...
{
private:
	LivingPerson *princess;                  /*Princess(living person object : isalive,name) is in one of the rooms of castle*/
	Items *items[5];                         /*items object array is created*/
	Monster *Monst;                          /*monster is also in one of the rooms*/
	char *description;                       /*shows description on every step of game*/
	int  NoOfItemsInRoom;                    /*items hide in rooms to get the clues to queen*/
	Room *Exits[4];                          /*total  exits of room*/
	char *ExitDir[4];                        /*total exit directions*/
	int TotalExits;                           /*counter to total exits*/
	int exitValue;                            /*counter*/


public:
	Room();
	~Room();
	void setPrincess(LivingPerson*);               /*set princess object of living person(is alive?)*/
	Items* returnKillngItem();                     /*return killing item of monster*/
	void setMemforExit();                          /*making memory after new exit from room*/
	char* getn();                                  /*getting name of monster*/

	void setAlive(bool);                             /*alive or dead monster*/
	void PrintMonsterDesc();                         /*printing description of monster (dead or alive)*/

	Monster* getMonster();                           /*return monster in the room to attack*/

	void setExitDir(int index, char*);               /*setting directions available for user in each room*/
	void setLockExitDir();                          /*setting locked directions*/


	void setNoOfItems(int);                       /*no of items in room*/

	Items* getItem(int);                           /*return no of items */

	bool ValidObject(Items*);                     /*return true if objects are there in the room*/

	Items* ReturnItem(char*);                     /*return item*/

	char* getDescription();                        /*description*/

	void setItems(Items *);                        /*set no of items in room*/
	 
	void setMonster(Monster*);                     /*setting monster in a room*/

	void setDescription(char*);                    /*description for user(guide)*/

	void setTotalExits(int);                       /*setting total exits*/

	int getTotalExits();                           /*getting total exits*/

	char* getExitDir();                            /*getting the direction selected from the user*/

	void setExits(int, Room*);                    /*setting no of exits available in a room*/

	void setLockExits();                            /*setting lock exits(user cant enter)*/

	int getExitValue(char *);                       /*getting the exit value*/

	bool ValidDirection(char*);                      /*checking valid direction*/

	Room* getExitRoom(char *);                        /*get the exit room*/
	int getNoOfitems();                              /*no of items total in rooms*/

	Items* CreateMemoryForItems(char *, char*, char*, int);                    /*runtime creating memory for items is room*/

	void VanishItem(Items *temp);                        /*remove any item after it is selected*/

	void AddItemFromBag(Items*);                         /*adding item*/

	void PrintItems();                                   /*print them*/
...
void Room::PrintMonsterDesc()                                /*prinitng monster description*/
{
	if (Monst != nullptr)
	{
...
void Room::setAlive(bool t)                                   /*setting monster alive or false*/
{
	Monst->setisalive(t);
}
Monster* Room::getMonster()                                   /*returning monster in the room in which it is found*/
{
	return Monst;
}
Items* Room::returnKillngItem()                                /*killing item return*/
{
...
void Room::PrintItems()                             /*just printing no of items if available to user*/
{
...
void Room::AddItemFromBag(Items *temp)                         /*adding an item in start when no item is there*/
{
	for (int i = 0; i<NoOfItemsInRoom; i++)
	{
		if (items[i] == NULL)
...
void Room::VanishItem(Items *temp)                      /*removing a specific item*/
{
	for (int i = 0; i<NoOfItemsInRoom; i++)
	{
		if (items[i] != NULL)
...
void Room::setMemforExit()                 /*just making memory*/
{
	Exits[0] = new Room;
	Exits[1] = new Room;
	Exits[2] = new Room;
...
Items* Room::ReturnItem(char* n)                    /*get golden egg (hint)*/
{
	...
	else if (strcmp(n, "shield") == 0)                         /*shield form monster(extra life)*/
	{
		for (int i = 0; i<NoOfItemsInRoom; i++)
		{
			if (items[i] != nullptr)
	...
	else if (strcmp(n, "chalice") == 0)                       /*a type of lamp*/
	{
		for (int i = 0; i<NoOfItemsInRoom; i++)
		{
			if (items[i] != nullptr)
	...
	else if (strcmp(n, "dagger") == 0)                        /*finding a knife*/
	{
		for (int i = 0; i<NoOfItemsInRoom; i++)
		{
			if (items[i] != nullptr)
...
bool Room::ValidObject(Items* item)                    /*checking if there is any item in room*/
{
	for (int i = 0; i<NoOfItemsInRoom; i++)
	{
		if (items[i] != NULL)
...
Items* Room::CreateMemoryForItems(char * n, char* t, char* w, int index)   /*setting name,type and worth*/
{
...
Room* Room::getExitRoom(char *direction)    /*getting the room which is exit*/
{
...
bool Room::ValidDirection(char *direction)              /*checking user entered valid direction*/
{
	for (int i = 0; i<4; i++)
	{
...
private:
	Items *Bag[10];                      
	bool HasPrincess;                     /*found or not (True or false)*/
